...
assert.deepEqual(oldCharset, { a: 'В§' });
```

Some services add an initial `utf8=вњ“` value to forms so that old Internet Explorer versions are more likely to submit the form as utf-8.
...
assert.deepEqual(detectedAsUtf8, { a: 'Гё' });
...
assert.deepEqual(detectedAsIso8859_1, { a: 'Гё' });
```
...
assert.deepEqual(detectedAsIso8859_1, { a: 'вє' });
```
...
```javascript
var iso = qs.stringify({ Г¦: 'Г¦' }, { charset: 'iso-8859-1' });
...
```javascript
var numeric = qs.stringify({ a: 'вє' }, { charset: 'iso-8859-1' });
assert.equal(numeric, 'a=%26%239786%3B');
```

You can use the `charsetSentinel` option to announce the character by including an `utf8=вњ“` parameter with the proper encoding if the checkmark, similar to what Ruby on Rails and others do when submitting forms.

```javascript
var sentinel = qs.stringify({ a: 'вє' }, { charsetSentinel: true });
assert.equal(sentinel, 'utf8=%E2%9C%93&a=%E2%98%BA');

var isoSentinel = qs.stringify({ a: 'Г¦' }, { charsetSentinel: true, charset: 'iso-8859-1' });
...
var shiftJISEncoded = qs.stringify({ a: 'гЃ“г‚“гЃ«гЃЎгЃЇпјЃ' }, { encoder: encoder });
...
assert.deepEqual(obj, { a: 'гЃ“г‚“гЃ«гЃЎгЃЇпјЃ' });
```
