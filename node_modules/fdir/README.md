...
вљЎ **The Fastest:** Nothing similar (in the NodeJS world) beats `fdir` in speed. It can easily crawl a directory containing **1 million files in < 1 second.**

рџ’Ў **Stupidly Easy:** `fdir` uses expressive Builder pattern to build the crawler increasing code readability.

рџ¤– **Zero Dependencies\*:** `fdir` only uses NodeJS `fs` & `path` modules.

рџ•є **Astonishingly Small:** < 2KB in size gzipped & minified.

рџ–® **Hackable:** Extending `fdir` is extremely simple now that the new Builder API is here. Feel free to experiment around.
