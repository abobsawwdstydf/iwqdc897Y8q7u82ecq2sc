...
вњЁ Support popular git providers (GitHub, GitLab, Bitbucket, Sourcehut) out of the box.

вњЁ Built-in and custom [template registry](#template-registry).

вњЁ Fast cloning using tarball gzip without depending on local `git` and `tar`.

вњЁ Works online and offline with disk cache support.

вњЁ Custom template provider support with programmatic usage.

вњЁ Support extracting with a sub dir.

вњЁ Authorization support to download private templates

вњЁ Optionally install dependencies after clone using [unjs/nypm](https://github.com/unjs/nypm)

вњЁ HTTP proxy support and native fetch via [unjs/node-fetch-native](https://github.com/unjs/node-fetch-native)
...
- `--force-clean`: вљ пёЏ Remove any existing directory or file recursively before cloning.
- `--shell`: вљ пёЏ Open a new shell with the current working directory in cloned dir. (Experimental).
...
- `options`: (object) Options are usually inferred from the input string. You can customize them.
  ...
  - `forceClean`: (boolean) вљ пёЏ Clean up any existing directory or file before cloning.
