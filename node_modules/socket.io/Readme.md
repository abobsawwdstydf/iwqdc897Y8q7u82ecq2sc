...
io.on('connection', socket => {
  socket.emit('request', /* вЂ¦ */); // emit an event to the socket
  io.emit('broadcast', /* вЂ¦ */); // emit an event to all connected sockets
  socket.on('reply', () => { /* вЂ¦ */ }); // listen to the event
});
```
...
io.on('connection', client => {
  client.on('event', data => { /* вЂ¦ */ });
  client.on('disconnect', () => { /* вЂ¦ */ });
});
server.listen(3000);
```
