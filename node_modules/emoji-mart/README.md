...
- [рџ’ѕ Data](#-data)
- [рџЏЄ Picker](#-picker)
- [рџ™ѓ Emoji component](#-emoji-component)
- [рџ•µпёЏвЂЌв™ЂпёЏ Headless search](#%EF%B8%8F%EF%B8%8F-headless-search)
- [рџ”¬ Get emoji data from native](#-get-emoji-data-from-native)
- [рџ—є Internationalization](#-internationalization)
- [рџ“љ Examples](#-examples)
- [рџ¤“ Built for modern browsers](#-built-for-modern-browsers)
...
- **Cons:** Network latency, doesnвЂ™t work offline (unless you configure a ServiceWorker)
...
| **noCountryFlags** | `false` | | Whether to show country flags or not. If not provided, tbhis is handled automatically (Windows doesnвЂ™t support country flags) |
...
## рџ™ѓ Emoji component
The emoji web component usage is the same no matter what library you use.

First, you need to make sure data has been initialized. You need to call this only once per page load. Note that if you call `init` like this, you donвЂ™t necessarily need to include data in your Picker props. It doesnвЂ™t hurt either, it will noop.
...
| **native** | `рџ‘Ќ` | A native emoji |
| **size** | `2em` | The inline element size |
| **fallback** | `:shrug:` | A string to be rendered in case the emoji canвЂ™t be found |
| **set** | `native` | The emoji set: `native`, `apple`, `facebook`, `google`, `twitter` |
| **skin** | `1` | The emoji skin tone: `1`, `2`, `3`, `4`, `5`, `6` |

## рџ•µпёЏвЂЌв™ЂпёЏ Headless search
...
search('christmas') // => ['рџЋ„', 'рџ‡Ёрџ‡Ѕ', 'рџ§‘вЂЌрџЋ„', 'рџ””', 'рџ¤¶', 'рџЋЃ', 'вѓпёЏ', 'вќ„пёЏ', 'рџЋ…', 'в›„']
```

## рџ”¬ Get emoji data from native
...
getEmojiDataFromNative('рџ¤ћрџЏї').then(console.log)
/* {
...
Given the small file size, English is built-in and doesnвЂ™t need to be provided.
