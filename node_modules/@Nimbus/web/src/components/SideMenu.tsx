...
export default function SideMenu({ isOpen, onClose }: SideMenuProps) {
  ...
  const menuItems = [
    ...
    { divider: true },
    { icon: Info, label: t('aboutApp'), subtitle: 'Nimbus Messenger v1.0', onClick: () => changeView('about') },
  ];
  ...
  const renderMain = () => (
    <motion.div key="main" className="flex flex-col h-full" initial={false} animate="center" exit="exit" variants={viewVariants} custom={-1} transition={{ duration: 0.2 }}>
      ...
      <div className="relative overflow-hidden flex-shrink-0">
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-Nimbus-500/40 via-purple-600/25 to-transparent pointer-events-none" />
        <div className="absolute -top-20 -right-20 w-56 h-56 bg-Nimbus-500/15 rounded-full blur-[80px] pointer-events-none" />
        ...
        <div className="relative p-6 pb-5">
          ...
          <div className="flex items-center gap-1.5 mt-1.5">
            <AtSign size={12} className="text-Nimbus-400" />
            <span className="text-sm font-medium text-Nimbus-100/70">{user?.username}</span>
          </div>
        </div>
        {/* Bottom fade line */}
      ...
      <div className="flex-1 overflow-y-auto px-3 py-4 space-y-1">
        {menuItems.map((item, i) => {
          ...
          return (
            ...
            >
              <div className="w-9 h-9 rounded-xl bg-white/[0.06] group-hover:bg-Nimbus-500/15 flex items-center justify-center transition-all duration-200 border border-white/[0.04] group-hover:border-Nimbus-500/20">
                <Icon size={17} className="text-zinc-400 group-hover:text-Nimbus-400 transition-colors duration-200" />
              </div>
              ...
              {'badge' in item && item.badge ? (
                <span className="bg-gradient-to-r from-Nimbus-500 to-purple-600 text-white text-[11px] font-bold min-w-[22px] h-[22px] px-1.5 rounded-full flex items-center justify-center flex-shrink-0 shadow-[0_0_12px_rgba(168,85,247,0.4)]">
                  {item.badge}
                </span>
              ) : (
    ...
    <motion.div key="profile" className="flex flex-col h-full" initial={{ x: 100, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 100, opacity: 0 }} transition={{ duration: 0.2 }}>
      ...
      <div className="flex items-center justify-between p-5 border-b border-white/5 bg-white/5 relative overflow-hidden flex-shrink-0">
        ...
        {!isEditing ? (
          ...
          <button onClick={handleSave} disabled={isSaving} className="relative z-10 p-2 rounded-full text-Nimbus-400 hover:text-Nimbus-300 hover:bg-Nimbus-500/10 transition-all border border-Nimbus-500/20">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="flex flex-col items-center pt-8 pb-4 px-6 relative overflow-visible">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[200px] h-[200px] bg-Nimbus-500/10 rounded-full blur-[80px] pointer-events-none" />
          ...
          <div className="relative group">
            ...
            >
              {avatarUploading ? (
                <Loader2 size={14} className="text-Nimbus-400 animate-spin" />
              ) : (
                <Camera size={14} className="text-Nimbus-400" />
              )}
            </button>
          ...
          {isEditing ? (
            <div className="mt-5 w-full max-w-[260px] relative">
              <div className="absolute -inset-0.5 bg-gradient-to-r from-Nimbus-500 to-purple-500 rounded-2xl opacity-50 blur-sm pointer-events-none" />
              <input
                type="text"
                value={displayName}
                onChange={(e) => setDisplayName(e.target.value)}
                placeholder={t('enterName')}
                className="relative text-lg font-bold text-center text-white bg-black/40 border border-white/20 outline-none px-4 py-2.5 w-full rounded-2xl transition-colors focus:bg-black/60 focus:border-Nimbus-400 placeholder-white/30"
              />
            </div>
          ) : (
          ...
          {/* Username badge */}
          <div className="flex items-center gap-1.5 mt-2 bg-Nimbus-500/10 hover:bg-Nimbus-500/20 transition-colors px-3.5 py-1.5 rounded-full border border-Nimbus-500/20 backdrop-blur-sm cursor-default">
            <AtSign size={13} className="text-Nimbus-400" />
            <span className="text-sm font-semibold text-Nimbus-100">{user?.username}</span>
          </div>
        </div>
        ...
        <div className="px-4 space-y-2.5 pb-6">
          ...
          <div className="bg-black/20 backdrop-blur-xl border border-white/5 rounded-2xl p-4 transition-all hover:bg-black/30 hover:border-white/10">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-semibold text-Nimbus-200/50 uppercase tracking-widest">{t('aboutMe')}</span>
            </div>
            {isEditing ? (
              <textarea
                value={bio}
                ...
                rows={3}
                className="w-full rounded-xl bg-black/40 text-sm text-white placeholder-white/30 p-3 border border-white/10 focus:border-Nimbus-500 transition-colors resize-none outline-none leading-relaxed"
          ...
          {user?.createdAt && (
            <div className="bg-black/20 backdrop-blur-xl border border-white/5 rounded-2xl p-4 transition-all hover:bg-black/30 hover:border-white/10">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs font-semibold text-emerald-200/50 uppercase tracking-widest">{t('onNimbusSince')}</span>
              </div>
              <p className="text-sm text-zinc-200 pl-1">
        ...
        {isEditing && (
          <div className="px-4 pb-6 flex gap-3">
            ...
            <button
              onClick={handleSave}
              disabled={isSaving}
              className="flex-1 py-3 rounded-xl bg-gradient-to-r from-Nimbus-500 to-purple-600 hover:from-Nimbus-600 hover:to-purple-700 text-sm font-bold text-white transition-all shadow-[0_0_20px_rgba(168,85,247,0.4)] flex items-center justify-center gap-2"
            >
              {isSaving ? <Loader2 size={16} className="animate-spin" /> : <Check size={16} />}
              {t('save')}
            </button>
      ...
      <div className="flex-1 overflow-y-auto py-2">
        ...
        <div className="px-5 py-3">
          ...
          <div className="space-y-1">
            <button
              onClick={() => setLang('ru')}
              className={`w-full flex items-center gap-4 px-3 py-3 rounded-xl transition-colors ${lang === 'ru' ? 'bg-Nimbus-500/15 ring-1 ring-Nimbus-500/30' : 'bg-surface-tertiary/50 hover:bg-surface-hover'}`}
            >
              <span className="text-lg">🇷🇺</span>
              <div className="flex-1 min-w-0 text-left">
                <p className="text-sm text-zinc-200">Русский</p>
              </div>
              {lang === 'ru' && <Check size={16} className="text-Nimbus-400" />}
            </button>
            <button
              onClick={() => setLang('en')}
              className={`w-full flex items-center gap-4 px-3 py-3 rounded-xl transition-colors ${lang === 'en' ? 'bg-Nimbus-500/15 ring-1 ring-Nimbus-500/30' : 'bg-surface-tertiary/50 hover:bg-surface-hover'}`}
            >
              <span className="text-lg">🇬🇧</span>
              <div className="flex-1 min-w-0 text-left">
                <p className="text-sm text-zinc-200">English</p>
              </div>
              {lang === 'en' && <Check size={16} className="text-Nimbus-400" />}
            </button>
          </div>
        </div>
        {/* Privacy */}
        <div className="px-5 py-3">
          ...
          <div className="space-y-1">
            ...
            >
              ...
              <div className={`w-10 h-6 rounded-full transition-colors flex items-center px-0.5 ${user?.hideStoryViews ? 'bg-Nimbus-500' : 'bg-zinc-600'}`}>
        ...
        <div className="px-5 py-3">
          ...
          <div className="flex items-center gap-4 px-3 py-3 rounded-xl bg-surface-tertiary/50">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm text-zinc-200">Nimbus Messenger</p>
                      ...
                      {friendSearchResults.map((u) => (
                        <div key={u.id} className="flex items-center gap-3 p-3 rounded-xl bg-white/5 border border-border/50">
                          {u.avatar ? (
                            ...
                            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-Nimbus-500 to-purple-600 flex items-center justify-center text-white font-bold text-sm">
                          ...
                          <button
                            ...
                            className="p-2 rounded-lg bg-Nimbus-500/20 text-Nimbus-400 hover:bg-Nimbus-500/30 transition-colors"
                            title={t('addFriend')}
                          >
                            <UserPlus size={16} />
                          </button>
  ...
  const renderAbout = () => (
    <motion.div key="about" className="flex flex-col h-full" initial={{ x: 100, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 100, opacity: 0 }} transition={{ duration: 0.2 }}>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
        <img src="/logo.png" alt="Nimbus" className="w-20 h-20 rounded-2xl object-cover mb-4 ring-2 ring-white/10" />
        <h2 className="text-xl font-bold gradient-text mb-1">Nimbus Messenger</h2>
