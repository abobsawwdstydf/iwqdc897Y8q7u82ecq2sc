...
import { playCallRingtone, stopCallRingtone, playCallEndedSound, playUserJoinSound, playUserLeaveSound } from '../lib/sounds';
...
export default function CallModal({ isOpen, onClose, targetUser, callType: initialCallType, incoming }: CallModalProps) {
  ...
  }, [isVideoOff, targetUserIdRef.current, localStreamRef.current, peerRef.current]);
  ...
  useEffect(() => {
    ...
    const onCallVideoToggle = async (data: { from: string; videoOn: boolean }) => {
      ...
      console.log('Remote video toggle:', data.videoOn);
    };
    ...
    socket.on('call_video_toggle', onCallVideoToggle);
    ...
    return () => {
      ...
      socket.off('call_video_toggle', onCallVideoToggle);
    ...
    <AnimatePresence>
      ...
      {isMinimized && callState === 'connected' ? (
        <motion.div
          ...
          className="fixed bottom-24 right-6 z-[100] flex items-center gap-3 px-4 py-3 rounded-2xl glass-strong shadow-2xl shadow-black/50 border border-white/10 cursor-pointer select-none"
