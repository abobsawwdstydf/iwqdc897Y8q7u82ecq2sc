  ...
  return (
    <>
      ...
      >
        ...
        {/* Аватар (чужие) — показываем только если это первое сообщение от автора подряд */}
        {!isMine && (
          <div className={`${showAvatar ? 'w-8 mr-2' : 'w-0 mr-0'} flex-shrink-0 self-end transition-all duration-200`}>
            {showAvatar ? (
...
function VideoPlayer({ src, isMine, filename, onOpenLightbox }: VideoPlayerProps) {
  ...
  return (
    <div
      className={`relative rounded-xl overflow-hidden bg-black group ${!isFullscreen ? 'max-h-80' : ''}`}
