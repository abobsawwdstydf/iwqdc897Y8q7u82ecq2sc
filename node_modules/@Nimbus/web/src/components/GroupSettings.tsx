...
import { Chat, UserPresence, ChatAvatar } from '../lib/types';
...
export default function GroupSettings({ chat, onClose }: GroupSettingsProps) {
  ...
  useEffect(() => {
    if (chat.id) {
      api.getChatAvatars(chat.id).then(setAvatars).catch(console.error);
    }
  }, [chat.id]);
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="flex flex-col items-center py-8 px-6">
            ...
            {/* Avatar Gallery for channels/groups */}
            {isAdmin && (
              <div className="mt-8 w-full">
                <AvatarGallery
                  ...
                  avatars={avatars}
                  onAvatarsChange={() => {
                    api.getChatAvatars(chat.id).then(setAvatars);
                    api.getChat(chat.id).then(updatedChat => updateChat(updatedChat));
                  }}
                />
              </div>
            )}
