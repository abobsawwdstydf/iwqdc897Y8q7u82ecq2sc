...
- [`formatDistance`](#formatdistance) вЂ” distance localizer function used by `formatDistance` and `formatDistanceStrict`.
- [`formatLong`](#formatlong) вЂ” contains long date localizer functions used by `format` and `formatRelative`.
- [`formatRelative`](#formatrelative) вЂ” relative date localizer function used by `formatRelative`.
- [`localize`](#localize) вЂ” contains functions, which localize the various date values. Required by `format` and `formatRelative`.
- [`match`](#match) вЂ” contains functions to parse date values. Required by `parse`.
- [`options`](#indexjs) вЂ” contains the index of the first day of the week for functions such as `startOfWeek`,
...
In this example, in Russian language a stand-alone month is in the nominative case ("СЏРЅРІР°СЂСЊ"),
and formatting month is in the genitive case ("СЏРЅРІР°СЂСЏ" as in "1-Рµ СЏРЅРІР°СЂСЏ"). Notice the different endings:

```js
// In `ru` locale:
var monthValues = {
  narrow: ["РЇ", "Р¤", "Рњ", "Рђ", "Рњ", "Р", "Р", "Рђ", "РЎ", "Рћ", "Рќ", "Р”"],
  abbreviated: [
    "СЏРЅРІ.",
    "С„РµРІ.",
    "РјР°СЂС‚",
...
and declension of word "РїСЂРѕС€Р»С‹Р№" which depends on the grammatical gender of the weekday:
...
function lastWeek(day) {
  ...
  switch (day) {
    case 0:
      return "'РІ РїСЂРѕС€Р»РѕРµ " + weekday + " РІ' p";
    case 1:
    case 2:
    case 4:
      return "'РІ РїСЂРѕС€Р»С‹Р№ " + weekday + " РІ' p";
    case 3:
    case 5:
    case 6:
      return "'РІ РїСЂРѕС€Р»СѓСЋ " + weekday + " РІ' p";
  }
}
...
var formatRelativeLocale = {
  ...
  yesterday: "'РІС‡РµСЂР° РІ' p",
  today: "'СЃРµРіРѕРґРЅСЏ РІ' p",
  tomorrow: "'Р·Р°РІС‚СЂР° РІ' p",
...
var matchDayPatterns = {
  narrow: /^[dlmДµjvs]/i,
  short: /^(di|lu|ma|me|(Дµ|jx|jh|j)a|ve|sa)/i,
  abbreviated: /^(dim|lun|mar|mer|(Дµ|jx|jh|j)a(Е­|ux|uh|u)|ven|sab)/i,
  wide: /^(diman(Д‰|cx|ch|c)o|lundo|mardo|merkredo|(Дµ|jx|jh|j)a(Е­|ux|uh|u)do|vendredo|sabato)/i,
};
var parseDayPatterns = {
  narrow: [/^d/i, /^l/i, /^m/i, /^m/i, /^(j|Дµ)/i, /^v/i, /^s/i],
  any: [/^d/i, /^l/i, /^ma/i, /^me/i, /^(j|Дµ)/i, /^v/i, /^s/i],
};
```

Here, for the word "dimanД‰o" the functions will match also "dimancxo", "dimancho"
...
// In `ru` locale:
var matchMonthPatterns = {
  narrow: /^[СЏС„РјР°РёСЃРѕРЅРґ]/i,
  abbreviated:
    /^(СЏРЅРІ|С„РµРІ|РјР°СЂС‚?|Р°РїСЂ|РјР°[Р№СЏ]|РёСЋРЅ[СЊСЏ]?|РёСЋР»[СЊСЏ]?|Р°РІРі|СЃРµРЅС‚?|РѕРєС‚|РЅРѕСЏР±?|РґРµРє)/i,
  wide: /^(СЏРЅРІР°СЂ[СЊСЏ]|С„РµРІСЂР°Р»[СЊСЏ]|РјР°СЂС‚Р°?|Р°РїСЂРµР»[СЊСЏ]|РјР°[Р№СЏ]|РёСЋРЅ[СЊСЏ]|РёСЋР»[СЊСЏ]|Р°РІРіСѓСЃС‚Р°?|СЃРµРЅС‚СЏР±СЂ[СЊСЏ]|РѕРєС‚СЏР±СЂ[СЊСЏ]|РѕРєС‚СЏР±СЂ[СЊСЏ]|РЅРѕСЏР±СЂ[СЊСЏ]|РґРµРєР°Р±СЂ[СЊСЏ])/i,
};
```
...
var matchDayPatterns = {
  narrow: /^[РІРїСЃС‡]/i,
  short: /^(РІСЃ|РІРѕ|РїРЅ|РїРѕ|РІС‚|СЃСЂ|С‡С‚|С‡Рµ|РїС‚|РїСЏ|СЃР±|СЃСѓ)\.?/i,
  abbreviated: /^(РІСЃРє|РІРѕСЃ|РїРЅРґ|РїРѕРЅ|РІС‚СЂ|РІС‚Рѕ|СЃСЂРґ|СЃСЂРµ|С‡С‚РІ|С‡РµС‚|РїС‚РЅ|РїСЏС‚|СЃСѓР±).?/i,
  wide: /^(РІРѕСЃРєСЂРµСЃРµРЅСЊ[РµСЏ]|РїРѕРЅРµРґРµР»СЊРЅРёРєР°?|РІС‚РѕСЂРЅРёРєР°?|СЃСЂРµРґ[Р°С‹]|С‡РµС‚РІРµСЂРіР°?|РїСЏС‚РЅРёС†[Р°С‹]|СЃСѓР±Р±РѕС‚[Р°С‹])/i,
};
```

(here, the `abbreviated` pattern will match both `РІСЃРє` and `РІРѕСЃ` as the short of `РІРѕСЃРєСЂРµСЃРµРЅСЊРµ` {Sunday})
...
```javascript
// In `ru` locale:
var matchOrdinalNumberPattern = /^(\d+)(-?(Рµ|СЏ|Р№|РѕРµ|СЊРµ|Р°СЏ|СЊСЏ|С‹Р№|РѕР№|РёР№|С‹Р№))?/i;
```
