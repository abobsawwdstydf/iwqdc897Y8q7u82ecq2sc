...
- [Added the Northern SГЎmi (`se`) locale.](https://github.com/date-fns/date-fns/pull/3724)
...
- Fixed bugs when working with negative numbers caused by using `Math.floor` (`-1.1` в†’ `-2`) instead of `Math.trunc` (`-1.1` в†’ `-1`). Most of the conversion functions (i.e., `hoursToMinutes`) were affected when passing some negative fractional input. Also, some other functions that could be possibly affected by unfortunate timezone/date combinations were fixed.
...
- **BREAKING**: Now all file contentвЂ™s exported via named exports instead of `export default`, which will require change direct imports i.e. `const addDays = require(вЂdate-fns/addDaysвЂ™)` to `const { addDays } = require(вЂdate-fns/addDaysвЂ™)`.
...
- **BREAKING**: Functions now donвЂ™t check the number of passed arguments, delegating this task to type checkers. The functions are now slimmer because of this.
...
- **BREAKING**: Functions that accept `Interval` arguments now do not throw an error if the start is before the end and handle it as a negative interval. If one of the properties in an `Invalid Date`, these functions also do not throw and handle them as invalid intervals.
  ...
  - `getOverlappingDaysInIntervals` now normalizes intervals before comparison, so `{ start: a, end: b }` is practically equivalent to `{ start: b, end: a }`. If any of the intervalsвЂ™ properties is an `Invalid Date`, the function will always return 0.

  - `isWithinInterval` now normalizes intervals before comparison, so `{ start: a, end: b }` is practically equivalent to `{ start: b, end: a }`. If any of the intervalsвЂ™ properties is an `Invalid Date`, the function will always return false.
...
- [Use the word "sekunda" instead of "vteЕ™ina" for second in the Czech locale.](https://github.com/date-fns/date-fns/pull/2577)
...
This release is brought to you by [Aleksei Korzun](http://github.com/a-korzun), [Tan75](http://github.com/tan75), [Rubens Mariuzzo](http://github.com/rmariuzzo), [Christoph Stenglein](http://github.com/cstenglein) and [ClГ©ment Tamisier](http://github.com/ctamisier).
...
- [Fixed minulГЅ ЕЎtvrtok in Slovak locale](https://github.com/date-fns/date-fns/pull/1701).
...
Huge thanks to [@lovelovedokidoki](https://github.com/lovelovedokidoki) who improved 8 (!) locales in an unstoppable open-source rampage and [@VesterDe](https://github.com/VesterDe) for fixing Slovenian locale рџ‘Џ
...
- **BREAKING** renamed ISO week-numbering year helpers:
  ...
  - `addISOYears` в†’ `addISOWeekYears`
  - `differenceInCalendarISOYears` в†’ `differenceInCalendarISOWeekYears`
  - `differenceInISOYears` в†’ `differenceInISOWeekYears`
  - `endOfISOYear` в†’ `endOfISOWeekYear`
  - `getISOYear` в†’ `getISOWeekYear`
  - `isSameISOYear` в†’ `isSameISOWeekYear`
  - `lastDayOfISOYear` в†’ `lastDayOfISOWeekYear`
  - `setISOYear` в†’ `setISOWeekYear`
  - `subISOYears` в†’ `subISOWeekYears`
...
- **BREAKING**: functions renamed:
  ...
  - `areRangesOverlapping` в†’ `areIntervalsOverlapping`
  - `eachDay` в†’ `eachDayOfInterval`
  - `getOverlappingDaysInRanges` в†’ `getOverlappingDaysInIntervals`
  - `isWithinRange` в†’ `isWithinInterval`
...
- **BREAKING**: functions renamed:
  ...
  - `distanceInWords` в†’ `formatDistance`
  - `distanceInWordsStrict` в†’ `formatDistanceStrict`
  - `distanceInWordsToNow` в†’ `formatDistanceToNow`
...
- [Hungarian locale (hu)](https://github.com/date-fns/date-fns/pull/503)
  (thanks to LГЎszlГі HorvГЎth [@horvathlg](https://github.com/horvathlg))
...
- Fixes for ordinals (1er, 2, 3, вЂ¦) in French locale.
...
- [Croatian locale (hr)](https://github.com/date-fns/date-fns/pull/365)
  (thanks to Matija MarohniД‡ [@silvenon](https://github.com/silvenon))
...
- [Turkish locale (tr)](https://github.com/date-fns/date-fns/pull/329)
  (kudos to Alpcan AydД±n [@alpcanaydin](https://github.com/alpcanaydin))
...
- [Portuguese locale (pt)](https://github.com/date-fns/date-fns/pull/316)
  (thanks to DГЎrio Freire [@dfreire](https://github.com/dfreire))

- [Swedish locale (sv)](https://github.com/date-fns/date-fns/pull/311)
  (thanks to Johannes UlГ©n [@ejulen](https://github.com/ejulen))
...
- Minor language fixes in the documentation.
  Thanks to Vedad Е oЕЎe [@vedadsose](https://github.com/vedadsose) ([#314](https://github.com/date-fns/date-fns/pull/314))
...
- Fixed broken test for Norwegian BokmГҐl locale.

## [1.14.0] - 2016-12-06

### Added

- [Norwegian BokmГҐl locale (nb)](https://github.com/date-fns/date-fns/pull/291)
