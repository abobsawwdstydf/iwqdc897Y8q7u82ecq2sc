...
```bash
# вњ… Exits with code 0 вЂ” second command exits first and succeeds
$ concurrently --success first 'sleep 1 && exit 1' 'exit 0'

# вќЊ Exits with a non-zero code вЂ” second command exits first, but with code 1
...
```bash
# вњ… Exits with code 0 - first command exits last and succeeds
$ concurrently --success last 'sleep 1 && exit 0' 'exit 1'

# вќЊ Exits with a non-zero code вЂ” first command exits last, but with code 1
