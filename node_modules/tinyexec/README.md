...
By default, tinyexec does not throw on nonвЂ‘zero exit codes. Check `result.exitCode` or pass `{throwOnError: true}`. 
...
The underlying Node.js `ChildProcess`. tinyexec keeps the surface minimal and does not reвЂ‘expose every child_process method/event. Use `proc.process` for advanced access (streams, events, etc.).
