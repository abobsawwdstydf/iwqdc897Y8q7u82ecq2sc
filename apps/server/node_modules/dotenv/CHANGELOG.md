...
* Add a new README section on dotenvвЂ™s approach to the agentic future.
...
- рџ™Џ A big thank you to new sponsor [Tuple.app](https://tuple.app/dotenv) - *the premier screen sharing app for developers on macOS and Windows.* Go check them out. It's wonderful and generous of them to give back to open source by sponsoring dotenv. Give them some love back.
...
```js
const TIPS = [
  'рџ”ђ encrypt with dotenvx: https://dotenvx.com',
  'рџ”ђ prevent committing .env to code: https://dotenvx.com/precommit',
  'рџ”ђ prevent building .env in docker: https://dotenvx.com/prebuild',
  'рџ› пёЏ  run anywhere with `dotenvx run -- yourcommand`',
  'вљ™пёЏ  specify custom .env file path with { path: \'/custom/path/.env\' }',
  'вљ™пёЏ  enable debug logging with { debug: true }',
  'вљ™пёЏ  override existing env vars with { override: true }',
  'вљ™пёЏ  suppress all logs with { quiet: true }',
  'вљ™пёЏ  write to custom object with { processEnv: myObject }',
  'вљ™пёЏ  load multiple .env files with { path: [\'.env.local\', \'.env\'] }'
]
```
...
- Default `quiet` to true вЂ“ hiding the runtime log message ([#874](https://github.com/motdotla/dotenv/pull/874))
...
- рџЋ‰ Added new sponsor [Graphite](https://graphite.dev/?utm_source=github&utm_medium=repo&utm_campaign=dotenv) - *the AI developer productivity platform helping teams on GitHub ship higher quality software, faster*.
...
- рџђћ Fix recent regression when using `path` option. return to historical behavior: do not attempt to auto find `.env` if `path` set. (regression was introduced in `16.4.3`) [#814](https://github.com/motdotla/dotenv/pull/814)

## [16.4.4](https://github.com/motdotla/dotenv/compare/v16.4.3...v16.4.4) (2024-02-13)

### Changed

- рџђћ Replaced chaining operator `?.` with old school `&&` (fixing node 12 failures) [#812](https://github.com/motdotla/dotenv/pull/812)
...
- Add `.env.vault` support. рџЋ‰ ([#730](https://github.com/motdotla/dotenv/pull/730))

в„№пёЏ `.env.vault` extends the `.env` file format standard with a localized encrypted vault file. Package it securely with your production code deploys. It's cloud agnostic so that you can deploy your secrets anywhere вЂ“В without [risky third-party integrations](https://techcrunch.com/2023/01/05/circleci-breach/). [read more](https://github.com/motdotla/dotenv#-deploying)
...
- _Breaking:_ Backtick support рџЋ‰ ([#615](https://github.com/motdotla/dotenv/pull/615))
...
- Properly parse empty single or double quoted values рџђћ ([#614](https://github.com/motdotla/dotenv/pull/614))
...
- Preserve backwards compatibility on values containing `#` рџђћ ([#603](https://github.com/motdotla/dotenv/pull/603))

## [14.3.1](https://github.com/motdotla/dotenv/compare/v14.3.0...v14.3.1) (2022-01-25)

### Changed

- Preserve backwards compatibility on exports by re-introducing the prior in-place exports рџђћ ([#606](https://github.com/motdotla/dotenv/pull/606))

## [14.3.0](https://github.com/motdotla/dotenv/compare/v14.2.0...v14.3.0) (2022-01-24)

### Added

- Add `multiline` option рџЋ‰ ([#486](https://github.com/motdotla/dotenv/pull/486))
...
- Add `override` option рџЋ‰ ([#595](https://github.com/motdotla/dotenv/pull/595))
...
- _Breaking:_ Support inline comments for the parser рџЋ‰ ([#568](https://github.com/motdotla/dotenv/pull/568))
