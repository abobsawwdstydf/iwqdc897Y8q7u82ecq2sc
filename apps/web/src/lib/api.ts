...
class ApiClient {
  ...
  async globalSearch(query: string) {
    return this.request<{ users: User[]; channels: Chat[] }>(`/users/search?q=${encodeURIComponent(query)}`);
  }
  ...
  async inviteToChannel(chatId: string, userIds: string[]) {
    return this.request<Chat & { addedUsers: string[] }>(`/chats/${chatId}/invite`, {
      method: 'POST',
  ...
  async getUserStories(userId: string) {
    return this.request<Story[]>(`/stories/user/${userId}/all`);
  }
