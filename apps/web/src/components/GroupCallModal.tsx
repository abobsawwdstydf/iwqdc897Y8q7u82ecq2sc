...
import { playUserJoinSound, playUserLeaveSound } from '../lib/sounds';
...
export default function GroupCallModal({ isOpen, onClose, chatId, chatName, callType: initialCallType }: GroupCallModalProps) {
  ...
  const createPeerConnection = useCallback(async (targetUserId: string, initiator: boolean) => {
    ...
    // Add local tracks (including screen share if active)
    if (localStreamRef.current) {
      localStreamRef.current.getTracks().forEach(track => pc.addTrack(track, localStreamRef.current!));
    }
    // Also add screen share track if screen sharing is active
    if (screenStreamRef.current) {
  ...
  const joinCall = useCallback(async () => {
    ...
    try {
      ...
      alert(errorMsg);
      joinedRef.current = false;
      // Cleanup without calling leaveCall to avoid circular dependency
      const socket = getSocket();
      socket?.emit('group_call_leave', { chatId });
      cleanup();
      onClose();
    }
  }, [chatId, initialCallType, cleanup, onClose]);
