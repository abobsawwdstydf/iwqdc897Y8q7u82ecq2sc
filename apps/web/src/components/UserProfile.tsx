...
type StoryTab = 'recent' | 'archive';

export default function UserProfile({ userId, channelId, chatId, onClose, isSelf, onStartCall, onOpenChat }: UserProfileProps) {
  ...
  const [activeStoryTab, setActiveStoryTab] = useState<StoryTab>('recent');
  ...
  // Load user stories when story tab changes
  useEffect(() => {
    if (userId && (activeTab as any) === 'stories') {
      loadUserStories();
    }
  }, [userId, activeStoryTab, activeTab]);
  ...
  const loadUserStories = async () => {
    ...
    try {
      ...
      // Filter: recent (last 48h) or archive (older)
      const now = Date.now();
      const hours48 = 48 * 60 * 60 * 1000;
      if (activeStoryTab === 'recent') {
        setUserStories(stories.filter(s => {
          const storyTime = new Date(s.createdAt).getTime();
          const isRecent = now - storyTime < hours48;
          const notExpired = new Date(s.expiresAt).getTime() > now;
          return isRecent && notExpired;
        }));
      } else {
        setUserStories(stories.filter(s => {
          const storyTime = new Date(s.createdAt).getTime();
          const isExpired = new Date(s.expiresAt).getTime() < now;
          const isOld = now - storyTime >= hours48;
                ...
                <div className="relative">
                  {channel.avatar ? (
                    ...
                    <div className="w-32 h-32 rounded-full bg-gradient-to-br from-surface to-surface-secondary flex items-center justify-center text-white font-bold text-4xl ring-4 ring-surface relative overflow-hidden">
                      ...
                      <span className="relative z-10 text-transparent bg-clip-text bg-gradient-to-br from-white to-zinc-400 drop-shadow-md">
                        {(channel.name || channel.username || '??').slice(0, 2).toUpperCase()}
                      </span>
                    </div>
                  )}
                </div>
            ...
            <div className="px-5 space-y-3 pb-8 relative z-10">
              ...
              <div className="bg-black/20 backdrop-blur-xl border border-white/5 rounded-2xl p-4 transition-all hover:bg-black/30 hover:border-white/10 group">
                ...
                <p className="text-sm text-zinc-200 pl-1">
                  {new Date(channel.createdAt).toLocaleDateString(lang === 'ru' ? 'ru-RU' : 'en-US', {
                    day: 'numeric',
                    month: 'long',
                    year: 'numeric',
                  })}
              ...
              <div className="flex px-2 pt-2 gap-1 overflow-x-auto no-scrollbar">
                {tabs.map((tab) => (
                  <button
                    ...
                    className={`flex-1 flex items-center justify-center gap-2 py-3 px-1 text-xs font-bold transition-all rounded-t-xl min-w-[100px] ${
                      activeTab === tab.key
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            <div className="flex flex-col items-center pt-8 pb-4 px-6 relative overflow-visible">
              ...
              <div className="relative group">
                ...
                <div className="relative">
                  {profile.avatar ? (
                    <img src={profile.avatar} alt="" className="w-32 h-32 rounded-full object-cover ring-4 ring-surface bg-surface" />
                  ) : (
                    <div className="w-32 h-32 rounded-full bg-gradient-to-br from-surface to-surface-secondary flex items-center justify-center text-white font-bold text-4xl ring-4 ring-surface relative overflow-hidden">
                      <div className="absolute inset-0 bg-gradient-to-tr from-accent/20 to-purple-500/20" />
              ...
              {/* РљРЅРѕРїРєРё РґРµР№СЃС‚РІРёР№ (Р·РІРѕРЅРѕРє, С‡Р°С‚) */}
              {!isSelf && userId && (
                <div className="mt-4 flex items-center justify-center gap-3">
                  <button
                    onClick={() => onOpenChat?.(userId)}
                    className="flex items-center gap-2 px-4 py-2.5 rounded-full bg-Nexo-500/20 border border-Nexo-500/30 text-Nexo-300 hover:bg-Nexo-500/30 transition-all text-sm font-medium"
                  >
                    <MessageCircle size={16} />
                    {t('chat')}
                  </button>
                  <button
                    onClick={() => onStartCall?.(userId, 'voice')}
                    className="flex items-center gap-2 px-4 py-2.5 rounded-full bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/30 transition-all text-sm font-medium"
                  >
                    <Phone size={16} />
                  </button>
                  <button
                    onClick={() => onStartCall?.(userId, 'video')}
                    className="flex items-center gap-2 px-4 py-2.5 rounded-full bg-blue-500/20 border border-blue-500/30 text-blue-400 hover:bg-blue-500/30 transition-all text-sm font-medium"
                  >
                    <Video size={16} />
                  </button>
                </div>
            ...
            <div className="px-5 space-y-3 pb-8 relative z-10">
              ...
              <div className="bg-black/20 backdrop-blur-xl border border-white/5 rounded-2xl p-4 transition-all hover:bg-black/30 hover:border-white/10 group">
                ...
                <p className="text-sm text-zinc-200 leading-relaxed pl-1">
                  {profile.bio || (<span className="text-white/30 italic">{t('notSpecified')}</span>)}
                </p>
              </div>
