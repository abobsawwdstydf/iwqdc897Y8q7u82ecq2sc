...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  addMessage: (message) => {
    ...
    set((state) => {
      ...
      // For scheduled messages or messages with temp IDs, remove the optimistic message
      const filteredMessages = chatMessages.filter((m) => {
        // Remove temp message with same content (optimistic UI)
        if (m.id?.startsWith('temp_') && m.content === message.content && m.senderId === message.senderId) {
          return false;
        }
        return true;
      });
